...
public class ProfileStarredViewHolder extends BaseViewHolder<GetStarredReposQuery.Node> {
    ...
    public static ProfileStarredViewHolder newInstance(ViewGroup parent, BaseRecyclerAdapter adapter, NumberFormat mNumberFormat) {
        return new ProfileStarredViewHolder(getView(parent, R.layout.repos_row_no_image_item), adapter, mNumberFormat);
    }

    private ProfileStarredViewHolder(@NonNull View itemView, @Nullable BaseRecyclerAdapter adapter, NumberFormat mNumberFormat) {
        super(itemView, adapter);
        this.mNumberFormat = mNumberFormat;
    }

    @Override
    public void bind(@NonNull GetStarredReposQuery.Node node) {
        if (node.isPrivate()) {
